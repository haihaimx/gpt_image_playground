...
export default function Toast() {
  ...
  const submissionProgress = useStore((s) => s.submissionProgress)

  if (!toast && !submissionProgress) return null

  // 提交进度条
  if (submissionProgress) {
    const percent = submissionProgress.phase === 'submitting'
      ? 100
      : Math.round((submissionProgress.current / submissionProgress.total) * 100)
    const label = submissionProgress.phase === 'submitting'
      ? '正在提交任务...'
      : `正在上传图片 ${submissionProgress.current}/${submissionProgress.total}`

    return (
      <div className="fixed bottom-24 left-1/2 z-[120] pointer-events-none toast-enter">
        <div className="flex flex-col gap-2 w-64 max-w-[calc(100vw-32px)] px-5 py-4 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border border-gray-200/60 dark:border-white/[0.08] rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] ring-1 ring-black/5 dark:ring-white/10">
