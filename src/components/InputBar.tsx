...
import { isBackendMode } from '../lib/backendTask'
...
export default function InputBar() {
  ...
  const hasSubmitApiConfig = isBackendMode() || Boolean(activeProfile.apiKey)
  const canSubmit = Boolean(prompt.trim() && hasSubmitApiConfig && !activeAgentIsRunning)
  const submitButtonAriaLabel = activeAgentIsRunning
    ? '停止生成'
    : maskDraft ? '遮罩编辑' : '生成图像'
  const submitTooltipText = activeAgentIsRunning ? '停止生成' : ''
