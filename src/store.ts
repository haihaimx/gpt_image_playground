...
import { isBackendMode, uploadImageToBackend, submitTaskToBackend, waitForTaskCompletion, fetchBackendImageDataUrl } from './lib/backendTask'
...
export type SettingsTab = 'general' | 'agent' | 'data' | 'about'
...
interface AppState {
  ...
  // 提交进度
  submissionProgress: { current: number; total: number; phase: 'uploading' | 'submitting' } | null
  setSubmissionProgress: (progress: AppState['submissionProgress']) => void
      ...
      setAppMode: (appMode) => {
        ...
        if (activeProfile.provider === 'openai' && activeProfile.apiMode !== 'responses') {
          state.setConfirmDialog({
            ...
            message: `当前配置不支持 Agent 模式所需的 Responses API。请联系管理员配置支持 Responses API 的服务商。`,
            confirmText: '知道了',
            showCancel: false,
            action: () => {},
          })
        ...
        state.setConfirmDialog({
          ...
          message: `当前配置所属的服务商暂不支持 Agent 模式。请联系管理员配置支持 Responses API 的服务商。`,
          confirmText: '知道了',
          showCancel: false,
          action: () => {},
        })
...
export async function initStore() {
  ...
  for (const task of tasks) {
    ...
    // 恢复后端模式 running 任务的轮询
    if (
      task.backendTaskId &&
      task.status === 'running'
    ) {
      pollBackendTask(task.id, task.backendTaskId)
    }
  }
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  // 后端模式：通过后端 API 提交任务
  if (isBackendMode()) {
    return submitTaskViaBackend(options)
  }
...
/** 通过后端提交任务 */
async function submitTaskViaBackend(options: { allowFullMask?: boolean }) {
  const { settings, prompt, inputImages, maskDraft, params, showToast } = useStore.getState()
  ...
  try {
    const totalUploads = inputImages.length + (maskDraft ? 1 : 0)
    let uploaded = 0

    useStore.getState().setSubmissionProgress({ current: 0, total: totalUploads, phase: 'uploading' })

    // 上传输入图片到后端
    const inputImageBackendIds: number[] = []
    for (const img of inputImages) {
      uploaded++
      useStore.getState().setSubmissionProgress({ current: uploaded, total: totalUploads, phase: 'uploading' })
      const backendId = await uploadImageToBackend(img.dataUrl)
      if (backendId) {
        inputImageBackendIds.push(backendId)
      }
    }

    // 上传蒙版图片
    let maskImageBackendId: number | undefined
    if (maskDraft) {
      uploaded++
      useStore.getState().setSubmissionProgress({ current: uploaded, total: totalUploads, phase: 'uploading' })
      maskImageBackendId = await uploadImageToBackend(maskDraft.maskDataUrl) || undefined
    }

    useStore.getState().setSubmissionProgress({ current: totalUploads, total: totalUploads, phase: 'submitting' })

    // 提交任务到后端
    const backendTaskId = await submitTaskToBackend({
      modelId,
      prompt: prompt.trim(),
      params,
      inputImageBackendIds,
      maskImageBackendId,
    })

    if (!backendTaskId) {
      useStore.getState().setSubmissionProgress(null)
    ...
    showToast('任务已提交，正在生成中...', 'success')
    useStore.getState().setSubmissionProgress(null)

    // 异步轮询任务状态
    pollBackendTask(taskId, backendTaskId)
  } catch (err: any) {
    useStore.getState().setSubmissionProgress(null)
...
/** 轮询后端任务状态 */
async function pollBackendTask(localTaskId: string, backendTaskId: number) {
  try {
    const result = await waitForTaskCompletion(
      backendTaskId,
      (status) => {
        console.log(`任务 #${backendTaskId} 状态: ${status.status}`)
      },
      2000,
      300000
    )
    ...
    if (result.status === 'done') {
      // 下载图片并缓存到 IndexedDB
      const outputIds: string[] = []
      for (const imgId of result.outputImageIds) {
        const filePath = result.outputImageFiles?.[imgId]
        if (filePath) {
          try {
            const dataUrl = await fetchBackendImageDataUrl(filePath)
            const localImgId = await storeImage(dataUrl, 'generated')
            cacheImage(localImgId, dataUrl)
            outputIds.push(localImgId)
          } catch (imgErr) {
            console.error(`下载图片 #${imgId} 失败:`, imgErr)
            outputIds.push(String(imgId))
          }
        } else {
          outputIds.push(String(imgId))
        }
      ...
      useStore.getState().showToast(`生成完成，共 ${result.outputImageIds.length} 张图片`, 'success')
    } else {
      // 任务失败
      updateTaskInStore(localTaskId, {
        status: 'error',
      ...
      useStore.getState().showToast(result.errorMessage || '生成失败', 'error')
    }
  } catch (err: any) {
    // AbortError = 页面刷新导致的 fetch 中断，不标记为失败，让 initStore 恢复轮询
    if (err.name === 'AbortError') {
      console.log(`任务 #${backendTaskId} 轮询被中断（页面刷新），等待恢复...`)
      return
    }
    ...
    useStore.getState().showToast(err.message || '轮询任务状态失败', 'error')
  }
}
